...
pub fn json_handle_error(error: actix_web_validator::Error, _request: &HttpRequest) -> Error {
    match error {
        actix_web_validator::Error::JsonPayloadError(err) => {
            InternalError::from_response("Validation error", HttpResponse::BadRequest().json(json!({
                "error": err.to_string()
            }))).into()
        },
        actix_web_validator::Error::Validate(err) => handle_validation_errors(err),
        err => {
            println!("{:?}", err);
            ErrorInternalServerError("Internal Server Error (JSON)")
        },
    }
}
